...
class TOCProcessor:
    ...
    def remove_line_range(self, toc_start_line_, toc_end_line_):
        self.file_data = self.file_data[:toc_start_line_] + self.file_data[toc_end_line_:]

        return toc_start_line_, toc_start_line_

    def generate_toc_table(self):
        # format
        # line_num, heading_level, disp_txt, heading_link, heading_link_dup
        #### heading_link_dup is link accounting for duplicated header
        self.toc_table = []
    ...
    def remove_link(self, text):
        ...
        # give recurse processing in case there are multiple link in the line
        return self.remove_link(outtxt)

    def process_string_to_disp_txt(self, text):

        # While... if additional processing is needed
    ...
    def process_string_to_link(self, text):
        ...
        if append_ind == 0:
            ...
            return link, link + "-{:d}".format(append_ind)
    ...
    def build_toc_data(self):
        ...
        for entry in self.toc_table:
            self.toc_data.append(
                TOC_HEADING[entry[1]] + '[' + entry[2] + '](#' + entry[4] + ')')
    ...
    def build_doc_data(self, toc_start_line_, toc_end_line_):
        self.data_to_write = \
            self.file_data[:toc_start_line_] \
            + self.toc_data \
            + self.file_data[toc_end_line_:]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--location-identifier', metavar='string', type=str, required=False,
                        default="[TOC]",
                        help='Set the search string for TOC substitution location.')
    ...
    try:
        ...
        try:
            ...
            if response not in YES_RESPONSE_VAR:
